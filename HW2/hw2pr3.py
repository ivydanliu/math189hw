...
def find_RMSE(W, X, y):
    ...
    "*** YOUR CODE HERE ***"
    RMSE = np.sqrt( np.linalg.norm(y - predict(W, X), 2)**2/len(y) )
    "*** END YOUR CODE HERE ***"
    return RMSE
...
def RMSE_vs_lambda(X_train, y_train, X_val, y_val):
    ...
    W_list = []
    "*** YOUR CODE HERE ***"
    reg_list = np.sort( np.random.uniform(low=0.0, high=150.0, size=len(X_val)) )
    for regularization in reg_list:
        W_opt = linreg(X_train, y_train, reg=regularization)
        W_list.append( W_opt )
    ...
    for index in range(len(RMSE_list)):
        if RMSE_list[index] < minRMSE:
            reg_opt = reg_list[index]
            minRMSE = RMSE_list[index]
        index += 1
    "*** END YOUR CODE HERE ***"
    return reg_opt
...
def linreg_no_bias(X, y, reg=0.0):
    ...
    "*** YOUR CODE HERE ***"
    n = X.shape[0]
    A_mod = X.T @ (np.eye(n)-1./n)
    W_opt = np.linalg.solve(
    		A_mod @ X + reg * np.eye(A_mod.shape[0]),
    		A_mod @ y,
    )
...
def grad_descent(X_train, y_train, X_val, y_val, reg=0.0, lr_W=2.5e-12, \
    ...
    while np.linalg.norm(W_grad) > eps and np.linalg.norm(b_grad) > eps \
        ...
        "*** YOUR CODE HERE ***"
        train_rmse = np.sqrt( np.linalg.norm(
            (X_train @ W).reshape(-1,1) + b - y_train, )**2 / y_train.shape[0])

        obj_train.append( train_rmse )

        obj_val.append( np.sqrt ( np.linalg.norm(
            (X_val @ W).reshape(-1,1) + b - y_val, )**2 / y_val.shape[0] ))

        W_grad = ( (X_train.T @ X_train + reg * np.eye(X_train.shape[1]) ) @ W + X_train.T @ \
            (b - y_train)) / X_train.shape[0]

        b_grad = ( (X_train @ W).sum() - y_train.sum() + b * m_train ) / X_train.shape[0]
