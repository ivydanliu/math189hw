...
def NLL(X, y, W, reg=0.0):
	"""	This function takes in four arguments:
		...
		This function calculates and returns negative log likelmation of all entries in a numpy
			   array a
	...
	NLL = -np.sum( np.sum(np.multiply(y, np.log(mu)), axis=1), axis=0)+ reg * np.trace(X @ W)
	"*** END YOUR CODE HERE ***"
	return NLL
...
def accuracy_vs_lambda(X_train, y_train_OH, X_test, y_test, lambda_list):
	...
	for reg in lambda_list:
		...
		"*** YOUR CODE HERE ***"
		W_opt, nll_list_gd = grad_descent(X_train, y_train_OH, reg = reg, lr=1e-5, eps=1e-6, max_iter=500, print_freq = 20)
...
if __name__ == '__main__':
	...
	# NOTE: You DO NOT neßed to fill in any additional helper function for this
