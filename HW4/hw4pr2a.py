...
def NLL(X, y, W, reg=0.0):
	...
	"*** YOUR CODE HERE ***"
	mu = sigmoid( X @ W )
	a = np.multiply(y, np.log(mu)) + np.multiply( (1. - y), np.log(1. - mu))
	nll = -a.sum() + reg * np.linalg.norm(W)**2 / 2
	"*** END YOUR CODE HERE ***"
	return nll
...
def newton_step(X, y, W, reg=0.0):
	...
	"*** YOUR CODE HERE ***"
	mu = sigmoid( X @ W )
	H = X.T @ np.diag( np.squeeze(mu*(1-mu) )) @ X + reg * np.eye(X.shape[1])
	d = np.linalg.solve( H, grad_logreg(X, y, W, reg=reg))
	"*** END YOUR CODE HERE ***"
	return d
...
def get_description(X, y, W):
	...
	accuracy = 1.* numCorrect/X.shape[0]
	precision = 1 * d/c
	recall = 1*b/a
...
def plot_description(X_train, y_train, X_test, y_test):
	...
	"*** YOUR CODE HERE ***"
	for reg in reg_list:
		W, nll_list_gd = grad_descent(X_train, y_train, reg = reg, lr = 2e-4, print_freq = 100)
	...
	for index in range(len(a_list)):
		if a_list[index] > maxAccuracy:
			maxAccuracy = a_list[index]
			reg_opt = index
	"*** END YOUR CODE HERE ***"
	return reg_opt
