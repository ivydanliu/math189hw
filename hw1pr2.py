...
if __name__ == '__main__':
    ...
    y_space = np.asarray(y_space).reshape(-1,1)
    "*** END YOUR CODE HERE ***"
    ...
    "*** YOUR CODE HERE ***"
    columnsOne = np.ones_like( X_space )
    X_space_stacked = np.hstack((columnsOne, X_space))
    XT = X_space_stacked.T
    W_opt = np.linalg.solve( XT @ X_space_stacked, XT @ y_space_rand )
    "*** END YOUR CODE HERE ***"
    ...
    "*** YOUR CODE HERE ***"
    X_space = np.linspace(0, 4, num=100).reshape(-1,1)
    y_pred_rand = [m_rand_opt*X + b_rand_opt for X in X_space]
    y_pred_rand = np.asarray(y_space).reshape(-1,1)
    "*** END YOUR CODE HERE ***"
