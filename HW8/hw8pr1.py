...
def k_means(X, k, eps=1e-10, max_iter=1000, print_freq=10):
	...
	while iter_num < max_iter:
		...
		"*** YOUR CODE HERE ***"
		dist = np.sqrt((( X - clusters[:,np.newaxis] )**2).sum(axis=2))
		...
		for k in range(clusters.shape[0]):
			if len(X[label==k]) != 0:
				clusters[k] =  X[label==k].mean(axis=0)

		# clusters = np.array([ X[label==k].mean(axis=0) for k in range(clusters.shape[0])])
		"*** END YOUR CODE HERE ***"
...
if __name__ == '__main__':
	...
	"*** YOUR CODE HERE ***"
	k_list = range(1,21)
	Cost_vs_k_plot = plt.plot(k_list, cost_k_list, 'g^')
	"*** END YOUR CODE HERE ***"
